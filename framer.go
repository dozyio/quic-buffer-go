...
import (
	...
	"github.com/dozyio/quic-buffer-go/internal/utils/ringbuffer"
	"github.com/dozyio/quic-buffer-go/internal/wire"
	"github.com/dozyio/quic-buffer-go/quicvarint"
)
...
func (f *framer) Append(
	...
	f.mutex.Lock()
	// log.Printf("[FRAMER] Append called. Active streams in queue: %d", f.streamQueue.Len())

	// pop STREAM frames, until less than 128 bytes are left in the packet
	numActiveStreams := f.streamQueue.Len()
	for i := 0; i < numActiveStreams; i++ {
		if protocol.MinStreamFrameSize > maxLen {
			log.Printf("[FRAMER] Append: Breaking out of loop, because min stream")
			break
		}
