...
func (s *SendStream) write(p []byte) (bool /* is newly completed */, int, error) {
	...
	for {
		log.Printf("[SEND STREAM %d] Loop: dataForWriting len=%d, nextFrame nil=%v, finishedWriting=%v, finSent=%v", s.streamID, len(s.dataForWriting), s.nextFrame == nil, s.finishedWriting, s.finSent)
		var copied bool
		...
		if deadline.IsZero() {
			log.Printf("[FLOW] SendStream %d blocked, waiting for window update (writeOffset=%d, window=%d)", s.streamID, s.writeOffset, s.flowController.SendWindowSize())
			<-s.writeChan
		} else {
			select {
			case <-s.writeChan:
...
func (s *SendStream) popNewOrRetransmittedStreamFrame(maxBytes protocol.ByteCount, v protocol.Version) (_ *wire.StreamFrame, _ *wire.StreamDataBlockedFrame, hasMoreData bool) {
	...
	if len(s.dataForWriting) == 0 && s.nextFrame == nil {
		if s.finishedWriting && !s.finSent {
			s.finSent = true
			log.Printf("[SEND STREAM %d] Creating FIN frame at offset %d", s.streamID, s.writeOffset)
	...
	log.Printf(
		"[SEND STREAM %d] popNewOrRetransmittedStreamFrame returning. Frame created: %t, HasMoreData: %t",
		s.streamID,
		f != nil, // Will be true if a STREAM frame was successfully created
		hasMoreData,
	)
